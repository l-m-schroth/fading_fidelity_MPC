...
def plot_phase_space(Phi_t, mpc=None, closed_loop_traj=None, open_loop_plan=False, latexify=False, legend=False, number=None, save=False):
    ...
    if legend:
        ...
        plt.legend(handles=handles, loc='upper right', fontsize=legend_fontsize, framealpha=0.9)
    
    plt.grid()
    # Save figure if requested
    if save:
        dir_path = get_dir("plots")
        filename = dir_path / f"van_der_pol/vdp_trajectory_tracking_{number}.pdf"
