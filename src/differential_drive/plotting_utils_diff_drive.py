...
def plot_diff_drive_trajectory(y_ref, mpc=None, closed_loop_traj=None, open_loop_plan=None, 
    ...
    """
    if latexify:
        latexify_plot(fontsize=24)  # Assumes you have a latexify_plot() defined elsewhere
    ...
    plt.scatter(y_ref[0], y_ref[1], color='orange', marker='o', s=100, label="Goal State")
    dx_ref = 0.1 * np.cos(y_ref[2])
    dy_ref = 0.1 * np.sin(y_ref[2])
    plt.arrow(y_ref[0], y_ref[1], dx_ref, dy_ref, head_width=0.03, color='orange')
    ...
    if initial_state is not None:
        ...
        plt.scatter(x0, y0, color='grey', marker='o', s=100, label="Initial State")
        dx_init = 0.1 * np.cos(theta0)
        dy_init = 0.1 * np.sin(theta0)
        plt.arrow(x0, y0, dx_init, dy_init, head_width=0.03, color='grey')
    ...
    # Optionally visualize elliptical constraints (obstacles)
    ...
    if legend:
        legend_fontsize = 24
        plt.legend(loc='upper right', fontsize=legend_fontsize, framealpha=0.9)
