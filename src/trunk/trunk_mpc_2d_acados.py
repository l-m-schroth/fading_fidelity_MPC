...
from trunk.trunk_utils import compute_controls_casadi, compute_q_casadi
...
class TrunkMPC2D(ABC):
    """
    This class (and its subclasses) implement the trunk MPC in acados. The seperation into a subclass was done as there were additional 
    tasks tested with the trunk (configurations tracking) that did not make it into the thesis, which were implemented via additional subclasses.
    ...
    def __init__(self, trunk_mpc_2d_options: TrunkMPC2DOptions) -> None:
        ...
        self.map_controls = trunk_mpc_2d_options.map_controls # flag that decides whether to apply compute controls contols mapping 
        ...
        self.has_elliptical_constraints = trunk_mpc_2d_options.ellipse_centers is not None and trunk_mpc_2d_options.ellipse_half_axes is not None
        self.ellipse_centers = np.atleast_2d(trunk_mpc_2d_options.ellipse_centers)
        self.ellipse_half_axes = np.atleast_2d(trunk_mpc_2d_options.ellipse_half_axes)
        ...
        self.acados_ocp_solver = AcadosOcpSolver(
            self.ocp, json_file=f"acados_ocp_solver_chain_{self.n_string}_{self.N_list_str}_{self.dt_string}_{self.qp_solver}_{self.has_elliptical_constraints}.json"
            )
    ...
    def export_ocp(self):
        if self.multi_phase:
            ...
            for phase in range(self.n_phases-1):
                ...
                time_steps.append(1.0) # for transition stage, set to 1 to correct for the scaling of the stage cost with the time step
        ...
        # set ocp options 
        ocp.solver_options.qp_solver = self.qp_solver #'FULL_CONDENSING_QPOASES' 
    ...
    def create_sim_solver(self, multi_phase, stage=0):
        ...
        return AcadosSimSolver(acados_sim, json_file=f"acados_sim_solver_chain_{self.n_string}_{self.N_list_str}_{stage}_{self.dt_string}_{self.qp_solver}.json")
    ...
    def solve_ocp(self):
        ...
        if status != 0:
            pass # throwing error causes problems
    ...
    def export_chain_model(self, n):
        ...
        if self.map_controls and self.n_high > n: # map contol inputs to better capture the higher dimensional system
            u_mapped = compute_controls_casadi(
                u[0],
                u[1],
                2*self.ode_params_dict[f"{self.n_high}"]["l"],
                2*self.ode_params_dict[f"{n}"]["l"],
                self.ode_params_dict[f"{self.n_high}"]["c"],
                self.ode_params_dict[f"{n}"]["c"],
                self.ode_params_dict[f"{n}"]["gear"],
                self.n_high,
                n
            )
            model_name += f'_map_controls_{self.n_high}'  # Name the model
        else:
            u_mapped = u
        u_expanded = ca.vertcat( # expand u to actuate all joints
            ca.repmat(u_mapped[0], n // 2, 1),  # Repeat u[0] n_half times
            ca.repmat(u_mapped[1], n // 2, 1)  # Repeat u[1] n_half times
        )
    ...
    def export_transition_ocp_chain(self, n_high_dim, n_low_dim):
        ...
        #ocp.solver_options.tf = 1.0 # dummy value, maybe needed for compilation, TODO: but check that this deos not cause problems
        return ocp
...
class TrunkMPC2DEETracking(TrunkMPC2D):
    ...
    def export_chain_ocp(self, n):
        ...
        if self.has_elliptical_constraints:
            ocp = self._add_ellipsoidal_constraints_to_ocp(ocp, ee_pos[0], ee_pos[1])

        # initial state constraint 
        ocp.constraints.x0 = np.zeros((nx,1))

        # # Numerical issues occur in combination with the constraint. Make it soft as a trial to see if that helps
        # ocp.constraints.idxsh = np.array([0])  # Make x_ee constraint soft

        # # Define slack penalties
        # ocp.cost.zl = np.array([0.5])  # Linear penalty for violation
        # ocp.cost.zu = np.array([0.5])
        # ocp.cost.Zl = np.array([0.1])   # Quadratic penalty weight
        # ocp.cost.Zu = np.array([0.1])   # Quadratic penalty weight

        # # Define slack bounds
        # ocp.constraints.lsh = np.array([0.0])  # No negative slack
        # ocp.constraints.ush = np.array([0.0])  # Max 5cm violation allowed
    ...
    def _add_ellipsoidal_constraints_to_ocp(self, ocp, x1, x2):
        num_ellipses = self.ellipse_centers.shape[0]
        h_expr_list = []

        for i in range(num_ellipses):
            x1_c, x2_c = self.ellipse_centers[i, :]
            a, b = self.ellipse_half_axes[i, :]
        ...
        ocp.model.con_h_expr_e = ca.vertcat(*h_expr_list)
    ...
    def export_ocp_point_mass(self):
        ...
        if self.has_elliptical_constraints:
            ocp = self._add_ellipsoidal_constraints_to_ocp(ocp, ocp.model.x[0], ocp.model.x[1])
